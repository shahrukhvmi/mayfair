...
const StepSeven = ({ setHideSidebar }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (res !== null) {
          ...
          const product = pro && typeof pro === "object" && !Array.isArray(pro)
            ? { ...pro, qty: 0 }
            : {};
  ...
  useEffect(() => {
    ...
    if (items && Array.isArray(items.variations)) {
      const updatedVariations = items.variations.map((v, index) => {
        const isSelected =
          Array.isArray(cart) && cart.some(item => item.id === v.id);

        const cartItem =
          Array.isArray(cart) && cart.find(item => item.id === v.id);
  ...
  const getTotalSelectedQty = () =>
    variations.reduce((total, v) => total + v.qty, 0);
  ...
  const generateMessages = (variations, selectedDose) => {
    ...
    const selectedIndex = sortedVariations.findIndex(
      (item) => item.mgValue === selectedDose
    );
    const prevDose =
      selectedIndex > 0 ? sortedVariations[selectedIndex - 1] : null;

    messages[selectedDose] = `

      If you are taking ${name
      } for the first time, you will need to start the treatment on the ${lowestDose.mgValue
  ...
  const updatedItems = doses?.map((item) => {
    ...
    const messageObj = selectedMessages?.find(
      (msg) => msg?.id === item?.id
    );

    return {
      ...item,
      product_concent: messageObj
        ? messageObj.message
        : item.product_concent || "No message available",
    };
  });
      ...
      prev.map((dose, i) => {
        if (i === index) {
          ...
          if (totalQty > parseInt(product.allowed)) {
            ...
            const productConcent =
              messageObj?.message || dose.product_concent || "No message available";
  ...
  const calculateSubtotal = () => {
    ...
    const doseTotal = (Array.isArray(variations) ? variations : []).reduce(
      (total, dose) => total + (dose.qty || 0) * parseFloat(dose.price || 0),
      0
    );
  ...
  const handleSelect = (e, index) => {
    ...
    if (!variations[index].isSelected) {
      if (totalQtyExcludingCurrent + 1 <= product.allowed) {
        ...
        setVariations((prev) => {
          const updatedVariations = prev.map((dose, i) =>
            i === index ? { ...dose, isSelected: !dose.isSelected } : dose
          );
  ...
  const handleRemoveItem = (id) => {
    ...
    setVariations((prev) => {
      const updatedVariations = prev.map((dose) =>
        dose?.id === id ? { ...dose, isSelected: false } : dose
      );
      return updatedVariations;
    });
  };
  ...
  return (
    ...
    //   <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-screen-md mx-auto sm:px-8">
    //     <div className="grid grid-flow-row sm:flex-row gap-4 px-4">
    //       {/* Left Column (Main Content) */}
    //       <div className="flex-[2] md:px-4 py-10">
    //         <h1 className="text-2xl md:text-4xl text-center mb-6 hidden sm:block">Suggested Treatment</h1>
    ...
    //               id={dose.id}
    //               onClick={() => {
    //                 handleVariationClick(dose.name, index);
    //               }}
    ...
    //                 isSelected={dose.qty > 0}
    //                 onSelect={(e) => {
    //                   setRemoveSelectedAddon((prevState) => [
    //                     ...prevState,
    //                     { e, index },
    //                   ]);
    ...
    //         <h1 className="text-2xl md:text-4xl text-center my-6 sm:hidden block">Suggested Treatment</h1>
    ...
    //           </div>
    //         </div>
    //         <div className="bg-white p-3 sm:p-6 rounded-lg shadow-md sm:fixed mt-10 sm:mt-[100px] w-full sm:w-[450px]">
    //           <h2 className="text-lg font-semibold mb-4">Order Summary</h2>


    //           <div className="overflow-y-auto max-h-56 sm:max-w-full">
    //             <OrderSummary
    //               cart={cart}
                      ...
                      <Checkbox
                        {...register("terms", {
                          required: isExpiryRequired
                            ...
                            : false, // ❌ No validation if not required
                        })}
                        icon={
